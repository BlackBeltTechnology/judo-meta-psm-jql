...
def void stringLiterals() {
    var string = parser.parseString("'hello'")
...
def void booleanLiterals() {
    var literal = parser.parseString("true") as BooleanLiteral
    assertTrue(literal.isIsTrue)
    literal = parser.parseString("false") as BooleanLiteral
...
def void caseEquality() {
    ...
    BigInteger.valueOf(10).assertEquals(((integerCase.body as SwitchEqualsExpression).cases.get(0) as SwitchEqualsCase).condition.expressionValue)
    
    var transformedSwitch = "CASE self.product.name!toUpperCase WHEN 'HAMBURGER' THEN 0 WHEN 'HOTDOG' THEN 1 ELSE -1".parse as SwitchExpression
    "HOTDOG".assertEquals(((transformedSwitch.body as SwitchEqualsExpression).cases.get(1) as SwitchEqualsCase).condition.expressionValue)
...
def void transform() {
    ...
    "toUpperCase".assertEquals(stringExp.transformations.get(0).feature.name)
    val navigationExp = "self.description!length".parse as NavigationExpression
    "length".assertEquals(navigationExp.transformations.get(0).feature.name)
    val sumExp = "('hello'!length + 'world'!length)".parse as BinaryOperation
    "length".assertEquals((sumExp.leftOperand as StringLiteral).transformations.get(0).feature.name)

    val concatExp = "self.description!concatenate(self.copyright)".parse
    "copyright".assertEquals((concatExp.transformations.get(0).parameters.get(0) as NavigationExpression).features.get(0).name)
    
    val unaryExp = "-123.0!round!radix(16)".parse as UnaryOperation
    "-".assertEquals(unaryExp.operator)
    "round".assertEquals(unaryExp.transformations.get(0).feature.name)
    "radix".assertEquals(unaryExp.transformations.get(1).feature.name)    
    BigInteger.valueOf(16).assertEquals(unaryExp.transformations.get(1).parameters.get(0).expressionValue) 
}

@Test
def void transformLambda() {
        val filterExp = "
        ...
        "filter".assertEquals(filterExp.transformations.get(0).feature.name)
        val filterLambda = filterExp.transformations.get(0).lambda as LambdaExpression
    ...
    def void cast() {
        ...
        val fqLambda = fqExp.transformations.get(0).lambda as LambdaExpression
        "e".assertEquals(fqLambda.argument.name)
    }
    
    @Test
    def void typeTransformations() {
        val exp = "self.field!instanceof(Lib::MyType)".parse as NavigationExpression 
        "Lib::MyType".assertEquals((exp.transformations.get(0).parameters.get(0) as NavigationExpression).base)
    ...
    def Expression parse(String expressionText) {
        return parser.parseString(expressionText)
    }
